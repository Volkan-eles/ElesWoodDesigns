/**
 * update_july09_csv.js
 * Imports Arc Rocking Lounger Chair Plans and updates existing products'
 * images, prices, descriptions, and tags from EtsyListingsDownload (2).csv.
 */
...
const CSV_PATH = path.join(ROOT, 'EtsyListingsDownload (2).csv');
...
function determineCategory(title, tags) {
  ...
  if (t.includes('shed') || t.includes('firewood') || t.includes('pergola') || t.includes('swing') ||
      ...
      t.includes('rocking lounger') || t.includes('zero-gravity lounger')) return 'Outdoor';
...
for (const row of dataRows) {
  ...
  if (existingTitleMap.has(titleLower)) {
    ...
    let etsyUrl = '';
    if (titleLower.includes('arc rocking lounger') || titleLower.includes('zero-gravity lounger')) {
      etsyUrl = 'https://www.etsy.com/listing/4534796103/arc-rocking-lounger-chair-plans';
    }
