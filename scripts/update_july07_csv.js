/**
 * update_july07_csv.js
 * Imports DIY Wall Shelf Plans and updates existing products'
 * images, prices, descriptions, and tags from EtsyListingsDownload (1).csv.
 */
...
function determineCategory(title, tags) {
  ...
  if (t.includes('loft bed') || t.includes('murphy bed') || t.includes('murphy desk') ||
      ...
      t.includes('shoe cabinet') || t.includes('wall shelf') || t.includes('standing shelf')) return 'Bedroom';
...
for (const row of dataRows) {
  ...
  if (existingTitleMap.has(titleLower)) {
    ...
    let etsyUrl = '';
    if (titleLower.includes('wall shelf') || titleLower.includes('shoe rack plans')) {
      etsyUrl = 'https://www.etsy.com/listing/4533934502/diy-wall-shelf-plans-pdf-wood-shoe-rack';
    }
