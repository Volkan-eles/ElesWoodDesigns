/**
 * update_july09_desktop_csv.js
 * Copies EtsyListingsDownload (3).csv from Desktop to project root public folder,
 * imports the new folding wooden patio bar product, and updates existing products.
 */
...
const DESKTOP_CSV_PATH = 'C:/Users/vlkne/Desktop/woodcraft-plans/public/EtsyListingsDownload (3).csv';
const TARGET_CSV_PATH = path.join(ROOT, 'public', 'EtsyListingsDownload (3).csv');
const JSON_PATH = path.join(ROOT, 'data', 'etsy_products.json');

// Copy the CSV to project first
console.log('Copying CSV from Desktop...');
fs.copyFileSync(DESKTOP_CSV_PATH, TARGET_CSV_PATH);
...
function determineCategory(title, tags) {
  ...
  if (t.includes('shed') || t.includes('firewood') || t.includes('pergola') || t.includes('swing') ||
      ...
      t.includes('yard game') || t.includes('trash can') || t.includes('garbage bin') ||
      t.includes('folding wooden patio bar') || t.includes('patio bar')) return 'Outdoor';
...
// Parse CSV
const csvContent = fs.readFileSync(TARGET_CSV_PATH, 'utf8');
...
for (const row of dataRows) {
  ...
  if (existingTitleMap.has(titleLower)) {
    ...
    let etsyUrl = '';
    if (titleLower.includes('folding wooden patio bar') || titleLower.includes('patio bar blueprint')) {
      etsyUrl = 'https://www.etsy.com/listing/4535095362/outdoor-plan-pdf-folding-wooden-patio';
    }
