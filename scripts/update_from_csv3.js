 ...
 * update_from_csv3.js
 * 
 * Reads EtsyListingsDownload (3).csv and:
 * 1. Adds new products not yet in JSON
 * 2. Updates existing product data (images, prices, descriptions)
 */
...
function determineCategory(title, tags) {
  ...
  if (
    ...
    t.includes('digital') || t.includes('vintage') || t.includes('tracker') ||
    t.includes('colouring') || t.includes('coloring') || t.includes('soccer') ||
    t.includes('football office') || t.includes('tournament')
  ) return 'Digital';
  ...
  if (t.includes('loft bed') || t.includes('murphy') || t.includes('bedroom') || t.includes('storage bench') ||
      t.includes('bookshelf') || t.includes('bookcase') || t.includes('shoe rack') || t.includes('shoe carousel') ||
      t.includes('floating shelf') || t.includes('shelf')) return 'Bedroom';
...
// Known Etsy URLs for new products
const ETSY_URLS = {
  'world cup 2026 sweepstake kit pdf | football office pool game': 'https://www.etsy.com/listing/4517866478/world-cup-2026-sweepstake-kit-pdf',
  'world cup 2026 printable tracker for kids': 'https://www.etsy.com/listing/4518069796/world-cup-2026-printable-tracker-for',
  'world cup 2026 sweepstake kit – printable football office pool game': 'https://www.etsy.com/listing/4517866478/world-cup-2026-sweepstake-kit-pdf',
};

function findEtsyUrl(titleLower) {
  for (const [key, url] of Object.entries(ETSY_URLS)) {
    if (titleLower.includes(key.toLowerCase().slice(0, 40))) return url;
  }
  return '';
}
...
console.log('CSV3 rows:', dataRows.length);
...
for (const row of dataRows) {
  ...
  if (existingTitleMap.has(titleLower)) {
    ...
    if (price && Math.abs(price - (p.originalPrice || 0)) > 0.001) {
    ...
    if (imgs.length > 0 && p.image !== imgs[0]) {
      ...
      changed = true;
      console.log(`  🖼  Updated images: "${title.slice(0, 60)}"`);
    }
    ...
    // Determine Etsy URL
    let etsyUrl = '';
    if (titleLower.includes('world cup 2026 sweepstake kit pdf')) {
      etsyUrl = 'https://www.etsy.com/listing/4517866478/world-cup-2026-sweepstake-kit-pdf';
    } else if (titleLower.includes('world cup 2026 printable tracker for kids')) {
      etsyUrl = 'https://www.etsy.com/listing/4518069796/world-cup-2026-printable-tracker-for';
    } else if (titleLower.includes('world cup 2026 sweepstake kit – printable football office pool game')) {
      etsyUrl = 'https://www.etsy.com/listing/4517866478/world-cup-2026-sweepstake-kit-pdf';
    }
    ...
    console.log(`     Etsy: ${etsyUrl}`);
    console.log(`     Images: ${imgs.length} | Price: ${price} | Category: ${category}`);
  }
}
